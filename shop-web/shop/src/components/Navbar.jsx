  ...
  return (
    <nav className="bg-white shadow-md px-4 py-3">
      <div className="flex justify-between container mx-auto items-center">
        <Link to="/" className="text-xl font-bold text-black">Shop</Link>
        ...
        <div className="hidden md:flex space-x-4 items-center">
          ...
          <Link to="/cart" className="hover:text-blue-500">
            Cart {cartItemCount > 0 && (<span className="ml-1 text-xs bg-red-500 text-white px-2 py-0.5 rounded-full">{cartItemCount}</span>)}
          </Link>
          {isLoggedIn ? (
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden mt-2 space-y-2">
          ...
          {isLoggedIn ? (
            <Link to="/profile" className="block hover:text-blue-500">Profile</Link>
          ) : (
